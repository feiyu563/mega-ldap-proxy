...
func LdapLogin(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		password_base64 := base64.URLEncoding.EncodeToString([]byte(password))
		//判断用户名密码是否正确
		...
		if IsLogin {
			...
			cookiepass := &http.Cookie{Name: "password", Value: password_base64, MaxAge: maxage}
...
func LdapAuthCheck(r *http.Request) (IsLogin bool, authorization *Authorization) {
	...
	password_base64 := ck.Value
	password_byte, _ := base64.URLEncoding.DecodeString(password_base64)
